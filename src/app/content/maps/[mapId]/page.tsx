...
export default function MapDetailPage({
  ...
  const { data: map, isLoading } = api.content.maps.get.useQuery({ id: mapId });
  ...
  const updateMap = api.content.maps.update.useMutation({
    onSuccess: () => {
      ...
      setHasChanges(false);
      void utils.content.maps.get.invalidate({ id: mapId });
    },
    onError: (error) => {
  ...
  const createVersion = api.content.maps.createVersion.useMutation({
    onSuccess: (version) => {
      ...
      router.push(`/content/maps/${mapId}/versions/${version.id}`);
    },
    onError: (error) => {
      toast.error(error.message);
    },
  ...
  // Initialize form when map loads
  if (map && !hasChanges && formData.name === "") {
    setFormData({
      name: map.name,
                      ...
                      <tr key={version.id} className="hover:bg-slate-800/30">
                        ...
                        <td className="px-4 py-2">
                          <div className="flex gap-2">
                            <Link
                              href={`/content/maps/${mapId}/versions/${version.id}`}
        ...
        <div className="space-y-6">
          <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-6">
            ...
            <div className="space-y-3">
              <button
                onClick={() => archiveMap.mutate({ id: mapId, isArchived: !map.isArchived })}
