...
-- AlterTable ItemTemplate (add columns only if table and columns don't exist)
DO $$ 
BEGIN
...
-- AlterTable MonsterTemplate (only if table exists)
DO $$ 
BEGIN
...
-- AlterTable QuestTemplate (only if table exists)
DO $$ 
BEGIN
...
-- AlterTable MapZone (only if table exists)
DO $$ 
BEGIN
...
-- CreateIndex (only if tables exist and indexes don't exist)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='ItemTemplate') THEN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='GatheringNode') THEN
